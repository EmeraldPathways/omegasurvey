...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ surveys: surveys.map(({ id, title, status }) => ({ id, title, status })), activeSurveyId: survey.id, survey: { id: survey.id, title: survey.title, questions: survey.questions }, stats: { total: Number(statsRow?.total ?? 0), imported: Number(statsRow?.imported ?? 0), sent: Number(statsRow?.sent ?? 0), opened: Number(statsRow?.opened ?? 0), submitted: Number(statsRow?.submitted ?? 0) }, recipients: recipients.results, configuration: { emailReady: Boolean(bindings().RESEND_API_KEY && bindings().SENDER_EMAIL), senderEmail: bindings().SENDER_EMAIL ?? null } });
  } catch (error) {
